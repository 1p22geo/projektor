...
import { Container, Typography, Box, CircularProgress, Alert, Tabs, Tab, List, ListItem, ListItemText, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, TextField, Link } from '@mui/material';
...
const TeamModeration: React.FC = () => {
  ...
  const handleSaveUrl = async () => {
    try {
      await axios.put(`/api/teams/${teamId}`, { url: teamUrl });
      mutate(`/headteacher/teams/${teamId}`);
      setIsEditingUrl(false);
    } catch (error) {
      console.error('Failed to update team URL', error);
    }
  };
            ...
            <List>
              {messages?.map((msg: ChatMessage) => (
                ...
                >
                  <ListItemText
                    primary={msg.message}
                    secondary={`${msg.user?.name || msg.user?.email || 'Unknown'} - ${new Date(msg.created_at).toLocaleString()}`}
                  />
                </ListItem>
              ))}
            </List>
              ...
              {files?.map((file: TeamFile) => (
                ...
                >
                  <ListItemText
                    ...
                    secondary={`${(file.size / 1024).toFixed(2)} KB - ${file.user?.name || 'Unknown'}`}
                  />
                </ListItem>
              ))}
            </List>
